...
The last bit of modeling security is to associate employees with a manager. In this domain, an user can have one manager while a manager can have multiple employees:
...
Spring Security supports a multitude of options when it comes to defining security policies. In this section, you want to restrict things such that ONLY managers can view user payroll data, and that saving, updating, and deleting operations are confined to the user's manager. In other words, any manager can log in and view the data, but only a given user's manager can make any changes.
...
On `save()`, either the user's manager is null (initial creation of a new user when no manager has been assigned), or the user's manager's name matches the currently authenticated user's name. Here you are using http://docs.spring.io/spring-security/site/docs/current/reference/htmlsingle/#el-access[Spring Security's SpEL expressions] to define access. It comes with a handy "?." property navigator to handle null checks. It's also important to note using the `@Param(...)` on the arguments to link HTTP operations with the methods.

On `delete()`, the method either has access to the user, or in the event it only has an id, then it must find the *userRepository* in the application context, perform a `findOne(id)`, and then check the manager against the currently authenticated user.
...
The one wrinkle is that Spring Security is active with access rules in full force when this loader runs. Thus to save user data, you must use Spring Security's `setAuthentication()` API to authenticate this loader with the proper name and role. At the end, the security context is cleared out.
...
With all these mods in the backend, you can now shift to updating things in the frontend. First of all, show an user's manager inside the `<Employee />` React component:

[source,javascript,indent=0]
----
include::src/main/js/app.js[tag=user]
----

This merely adds a column for `this.props.user.entity.manager.name`.
...
If you try *Delete*, it should fail with a similar message. Create a new user, and it should be assigned to you.

== Review

In this section:

* You defined the model of manager and linked it to an user via a 1-to-many relationship.
...
* You wrote another Spring Data REST event handler to trap creation events before they happen so they current user could be assigned as the user's manager.
* You updated the UI to show an user's manager and also display error pop-ups when unauthorized actions are taken.
